...
# endfunction util:get_repository_name
#     set url (git remote get-url origin)
...
function trans -a url
    set cur_dir (pwd)

    cd ~/Downloads/youtube
    mkdir ~/Downloads/youtube

    set result (yt-dlp --write-auto-subs --sub-lang en --convert-subs lrc --skip-download --restrict-filenames $url)

    set filename (echo $result  \
    | sed -n 's/.*file//p' \
    | sed -E 's/(.*)\.vtt .*/\1.lrc/' \
    | string trim)
    ...
    echo $file | deepl translate >> $filename

    mv $filename "$filename.md"

    nvim -c "lua vim.api.nvim_input('gg')" -c "FormatText" -c "w" "$filename.md"
