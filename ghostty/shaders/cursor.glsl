...
const vec4 TRAIL_COLOR = vec4(0.365, 0.894, 0.780, 1.0);
const vec4 TRAIL_COLOR_ACCENT = vec4(0.365, 0.894, 0.780, 1.0);

const float DURATION = 0.7; //IN SECONDS
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // cursor xy has the postion in a space of -1 to 1;zw has the width and height
    ...
    float sdfCurrentCursor = getSdfRectangle(vu, currentCursor.xy - (currentCursor.zw * offsetFactor), currentCursor.zw * 0.2);
    ...
    float mod = .07 * (1.5 - easedProgress); // Modulate the sdfTrail to make it more visible at the start of the trail
    //trailblaze
    ...
    //cursorblaze
    //trail = mix(saturate(TRAIL_COLOR_ACCENT, 1.5), trail, 1. - smoothstep(0., sdfCurrentCursor + .002, 0.002));
    //trail = mix(saturate(TRAIL_COLOR, 1.5), trail, 1. - smoothstep(0., sdfCurrentCursor + .005, 0.002));
